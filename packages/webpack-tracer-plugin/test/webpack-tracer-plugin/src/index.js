...
function CEditorPage(api)
{
	...
	this.checkBodyOffset = function() {
		const offset = jQuery("#" + this.Name).offset();

		if (offset) {
			this.X = offset.left * AscCommon.AscBrowser.getAdjustedZoom();
			this.Y = offset.top * AscCommon.AscBrowser.getAdjustedZoom();
		}
	};
	...
	this.checkBodySize = function()
	{
        ...
        /*
        * На macOS при переключении из дочернего PPTX/XLSX контейнера в DOCX 'display:none' снимается чуть позже, чем мы расчитываем размеры контейнера.
        * В момент вызова checkBodySize() DOM‑элемент по-прежнему скрыт и offsetWidth/offsetHeight возвращают 0.
        * 
        * Это условие не даёт «обнулять» размеры, если до этого они уже были валидными.
        * т.е. мы сохраняем прежнюю ширину/высоту, и последующие функции (масштабирование канваса, позиционирование страниц) продолжают работать с правильными значениями.
        * Как только контейнер становится видимым и документ начинает отдавать реальные offsetWidth/offsetHeight, это условие не выполняется и размеры продолжают обновляться.
        */
        if ((_newW <= 0 || _newH <= 0) && (this.Width > 0 && this.Height > 0)) {
            return false;
        }
	...
	this.UpdateReaderContent = function()
	{
		...
		{
			this.ReaderModeDivWrapper.innerHTML = "<div id=\"reader_id\" style=\"width:100%;display:block;z-index:9;font-family: 'liberation sans';font-size:" +
	...
	this.EnableReaderMode = function()
	{
		...
		this.ReaderModeDivWrapper.setAttribute("style", "z-index:11;font-family:'liberation sans';font-size:12pt;position:absolute;\
	...
	this.OnPaint = function()
	{
		...
		else
		{
			...
			// Обновить список ранов в perm промежутке для подсветки // R7
			this.m_oDrawingDocument.UpdateStatePermEls && this.m_oDrawingDocument.UpdateStatePermEls(); // R7
			...
			for (let i = this.m_oDrawingDocument.m_lDrawingFirst; i <= this.m_oDrawingDocument.m_lDrawingEnd; i++)
			{
				...
				{
					/* Если вторым аргументом передать сюда context, то удобно наблюдать
					через отладчик как элементы отрисовываются в прямом эфире.
					 */
...
globalThis.g_fontApplication = g_fontApplication;
globalThis.AscBrowser = AscBrowser;
globalThis.g_anchor_left = g_anchor_left;
globalThis.g_anchor_top = g_anchor_top;
globalThis.g_anchor_right = g_anchor_right;
globalThis.g_anchor_bottom = g_anchor_bottom;
globalThis.global_keyboardEvent = global_keyboardEvent;
globalThis.global_mouseEvent = global_mouseEvent;
globalThis.g_dKoef_pix_to_mm = g_dKoef_pix_to_mm;
globalThis.g_dKoef_mm_to_pix = g_dKoef_mm_to_pix;
globalThis.Page_Width = Page_Width;
globalThis.Page_Height = Page_Height;
globalThis.X_Left_Margin = X_Left_Margin;
globalThis.X_Right_Margin = X_Right_Margin;
globalThis.Y_Bottom_Margin = Y_Bottom_Margin;
globalThis.Y_Top_Margin = Y_Top_Margin;
globalThis.X_Right_Field = X_Right_Field;
globalThis.Y_Bottom_Field = Y_Bottom_Field;
globalThis.tableSpacingMinValue = tableSpacingMinValue;
globalThis.CEditorPage = CEditorPage;
