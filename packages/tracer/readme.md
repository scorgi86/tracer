...
**Решение:** явное наблюдение свойств через `observeProperties` помогает зафиксировать конкретные `get`/`set` события и связать их с цепочкой вызовов. Вместо общего шума разработчик видит, где именно произошло нужное изменение.

```js
Tracer.observeProperties(target, { name: "Order", properties: "status" });

Tracer.trace((event) => {
  console.log(event.fullName, event.curValue, "->", event.value);
}, { eventTypes: "properties", property: "status" });
```
...
**Решение:** `Tracer.observeProperties(...)` в `deep` режиме позволяет повесить наблюдение на вложенные свойства и получать события по полному пути.

```js
Tracer.observeProperties(color, { name: "Color", properties: true, deep: true });
...
- Ручное наблюдение свойств выполняется через единую точку `observeProperties`; события можно сузить фильтрами `Tracer.trace`.
...
- `Tracer.observeProperties(target, options)` — единая явная точка наблюдения свойств.
- `options.properties` — имя свойства, список имен или `true` для всех собственных свойств.
- `options.deep` / `options.maxDepth` — наблюдение вложенных объектов.
- `options.useProxy` — proxy-режим, когда нужно заменить ссылку на наблюдаемый объект.

`enableProperties` в конфиге разрешает генерацию событий для уже инструментированных свойств, но не делает объект наблюдаемым само по себе. Для instrumentation используйте `observeProperties`.

### Подписки на события трассировки

- `Tracer.trace(callback, options?)` — каноническая подписка на события трассировки; возвращает функцию отписки.
- `options.eventTypes` — `"all"`, `"calls"`, `"properties"`, конкретный event type или список.
- `options.property`, `options.fullName`, `options.className`, `options.fnKey`, `options.place` — фильтры событий.
- `options.slice` / `options.sliceSequence` — фильтрация по активным слайсам.
- `options.batch` — батчинг событий.

Старые методы сохранены как совместимые wrappers без warning:
...
- `Tracer.traceProperty(propSelector, callback)` — property-события по имени/списку/predicate.
...
- `Tracer.printRegisteredSlices()` — вывести список зарегистрированных слайсов в консоль.
